...
# ガウス過程回帰
def gaussian_process_regression(x, y, x_new, alpha=0.5, length_scale=1.0, noise_intensity=1e-10):
    """
    ガウス過程回帰による予測分布を求める
...
if __name__ == '__main__':
    ...
    y = np.sin(x) + 10e-5 * np.random.randn(len(x))
    split_position = -10
    ...
    y_pred, sigma_pred = gaussian_process_regression(x_known, y_known, x_unknown, noise_intensity=1e-8)
